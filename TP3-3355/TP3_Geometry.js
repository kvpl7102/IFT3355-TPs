...
TP3.Geometry = {
  ...
  hermite: function (h0, h1, v0, v1, t) {
    ...
    // hermite functions
    let x1 =  2 * t**3 - 3 * t**2 + 1;
    let x2 = -2 * t**3 + 3 * t**2;
    let x3 =      t**3 - 2 * t**2 + t;
    let x4 =      t**3 -     t**2;

    let x = x1 * h0.x + x2 * v0.x + x3 * h1.x + x4 * v1.x
    let y = x1 * h0.y + x2 * v0.y + x3 * h1.y + x4 * v1.y
    let z = x1 * h0.z + x2 * v0.z + x3 * h1.z + x4 * v1.z

    const p = new THREE.Vector3(x, y, z);
    
    const ptest2 = h0.multiplyScalar(x1)
    ...
    var ptest1 = new THREE.Vector3().addScaledVector(h0,x1);
    p.addScaledVector(h1, x2);
    p.addScaledVector(v0, x3);
    p.addScaledVector(v1, x4);
    console.log("###########################")
    console.log("ptest1: ")
    console.log(ptest1)
